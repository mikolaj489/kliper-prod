document.addEventListener('DOMContentLoaded', () => {
    ...
    let multiplier = 1;
    let ticking = false;
    let lineTop = 0, lineHeight = 0;
    let iconPositions = [];

    const getLineSpacing = () => {
        const raw = parseFloat(getComputedStyle(line).getPropertyValue('--line-spacing'));
    ...
    const recalcAll = () => {
        ...
        multiplier = Math.min(MIN_MULT + ratio * (MAX_MULT - MIN_MULT), MAX_MULT);
    ...
    const updateLineHeight = () => {
        ...
        const progress = Math.min(1, Math.max(0, window.scrollY / maxScroll) * multiplier);
        line.style.setProperty('--scroll-progress', progress);

        const lineEnd = lineTop + progress * lineHeight;
        iconPositions.forEach(({ el, top }) => el.classList.toggle('is-visible', lineEnd >= top));
