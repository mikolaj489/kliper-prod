document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!releasesContainer && !document.querySelector('.album-card')) return;
    ...
    function updateButtons(container) {
        ...
        if (next) next.disabled = currentIndex === allCards.length - 1;
    }
    ...
    function handleTracksToggle(toggle) {
        const expanded = toggle.dataset.expanded === 'true';
        const parent = toggle.closest('.album-card__tracks--expanded');
        if (!parent) return;
        const list = parent.querySelector('.album-card__tracks-list');
        if (!list) return;
